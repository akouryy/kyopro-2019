...
/*?^graph.hpp*/
TL<TN V,TN E>
struct NibuGraph{Graph<V,E>g;VB colors;
NibuGraph(int nv,CS VB&colors):g(nv),colors(colors){}
void add_uedge(int i,int j,CS E&val){
if(debug&&colors[i]==colors[j]){
cerr<<"uedge between same color:("<<i<<","<<j<<"),color="<<colors[i]<<")" ln;
exit(1);
}
g.add_uedge(i,j,val);
}
Graph<V,E>matcher(CS E&one)CS{int n=g.nv(),s=n,t=n+1;Graph<V,E>m(n+2);
times(n,i){if(colors[i]){m.add_dedge(i,t,one);}else{m.add_dedge(s,i,one);
}
for(CS Edge<E>&e:g.edges[i]){if(colors[i]){m.add_dedge(e.to,i,one);
}else{m.add_dedge(i,e.to,one);
}}}
RT m;}};
/*?^graph_nibu.hpp*/
/*?flow.hpp*/
...
/* </foxy.memo-area> */
  ...
  times(N * 2, c) {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  nibu.g.nue(): "<<(nibu.g.nue())ln<<"  f:            "<<(f)ln;}
    match += f;
  }
