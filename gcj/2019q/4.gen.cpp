...
#define upto(f, t, i)    for(auto rab_base_dest = (t), i = (remove_reference<decltype(t)>::type)(f); i <= rab_base_dest; i++)
#define uptil(f, t, i)   for(auto rab_base_dest = (t), i = (remove_reference<decltype(t)>::type)(f); i <  rab_base_dest; i++)
#define downto(f, t, i)  for(auto rab_base_dest = (remove_reference<decltype(f)>::type)(t), i = (f); i >= rab_base_dest; i--)
#define downtil(f, t, i) for(auto rab_base_dest = (remove_reference<decltype(f)>::type)(t), i = (f); i >  rab_base_dest; i--)
...
using VI = vec<int>; using VC = vec<char>; using VS = vec<string>; 
/* </rab:include(typedefs.hpp)> */
...
/* </foxy.memo-area> */
  ...
  if(debug) cerr << "pieces: " << (pieces) ln;
  VS pyon(4);
  times(4, f) {
    int sz = 1 << (3 - f);
